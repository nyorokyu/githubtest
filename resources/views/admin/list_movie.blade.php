...
@section('title', '動画投稿一覧')
@include('admin.layouts.head')

@section('h1', '動画投稿一覧')
@section('header_nav')
  <p><a href="{{ route('admin.movie.index') }}" class="btn">新規追加</a></p>
        ...
        @foreach($list as $data)
          <tr>
            ...
            @else
              <td>{{$data->title}}</td>
            @endif
            <td>{{$data->movieCat->category_name}}</td>
            <td>{{$data->displayed_at ?? $data->displayed_at_hidden}}</td>
            <td class="w-200">
              <form action="{{route('admin.list_movie.destroy', $data->id)}}" method="POST" class="form">
                @csrf
                @method('DELETE')
                <div class="box-flex">
                  <a href="{{route('admin.movie.index', $data->id)}}" class="btn">変更</a>
